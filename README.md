...
3. **Canned scenarios** — keyword match on the description when the model returns `null`; also the
   demo chips.
4. **Service worker** — `public/sw.js` precaches the shell and data files (version string `VERSION` in `sw.js`, currently `ckh-v9`), cache-first
...
- The service worker version string (`ckh-v9` in `sw.js`) must be bumped when cached files change
